...
"""The module is responsible for gathering data from a device."""
...
class EmotivPacket(object):
    """A class that represents data packet reveived from Emotiv EPOC.
    
    It is used to handle communication with the device.
    """
    ...
    def __recovery__(self):
        """ Used for dictionary depickling. """
        for name, bits in sensorBits.items():
            value = self.get_level(self.rawData, bits)
            setattr(self, name, (value,))
    ...
    def __getstate__(self):
        """ Returns a string that represents single packet. """
        state = {}
        state['rawData'] = self.rawData
        state['counter'] = self.counter
        state['battery'] = self.battery
        state['sync'] = self.sync
        state['gyroX'] = self.gyroX
        state['gyroY'] = self.gyroY
        state['sensors'] = json.dumps(self.sensors.copy())
        return state
        
        
    def __setstate__(self, state):
        """ Converts string to a packet. """
        self.rawData = state['rawData']
        self.counter = state['counter']
        self.battery = state['battery']
        self.sync = state['sync']
        self.gyroX = state['gyroX']
        self.gyroY = state['gyroY']
        self.sensors = json.loads(state['sensors'])
        self.__recovery__()

    def get_level(self, data, bits):
        """ Gets sensor value from raw Emotiv data. """
    ...
    def handle_quality(self, sensors):
        """ Gets sensor quality from raw Emotiv data. """
    ...
    def battery_percent(self):
        return int(self.battery / 2.56)

    def __repr__(self):
        """ Returns EmotivPacket. """
...
class Emotiv(object):
    """ Class that represents Emotiv EPOC device, used to establish connection with hardware."""
    channels = ['F3','F4','P7','FC6','F7','F8','T7','P8','AF4','T8','AF3','O2','O1','FC5']
    """A list of EEG channels that is used in many places."""
    ...
    def setup(self, headsetId=0):
        """Sets the device up. Designed both for Windows and Linux, currently enabled only for Windows. [by emokit creators]"""
        self.setupWin()
        '''
        if windows:
            self.setupWin()
    ...
    def updateStdout(self):
        """Shows updated information about readings in the console."""
        while self._goOn:
    ...
    def getLinuxSetup(self):
        """Sets the Linux configuration up. Currently not used. Not tested. [by emokit creators]"""
        rawinputs = []
    ...
    def setupWin(self):
        """Sets the Windows configuration up. [by emokit creators]
         
        Runs a loop ending upon termination of the program."""
        devices = []
        try:
    ...
    def setupPosix(self):
        """Sets the POSIX configuration up, upon successful configuration, returns True. Not tested. [by emokit creators]"""
    ...
    def setupCrypto(self, sn):
        """Sets the decryption tool up. Emotiv uses AES. [by emokit creators]
            
        **Keyword arguments:**
            * sn -- serial number of the device (provided by the main setup functions)

        Runs an infinite decryption loop."""
        type = 0 #feature[5]
        type &= 0xF
        type = 0
    ...
    def dequeue(self):
        """Picks one packet from a packet queue. [by emokit creators]"""
        try:
    ...
    def close(self):
        """Turns the device handling off. Works differently on Windows and POSIX. [by emokit creators]"""
        if windows:
            self.device.close()
        else:
            self._goOn = False
